...
sub create{
  ...
  my ($dst, $newFn)=$self->new_filename($splitFn, $mdat);
...
#  if(-f $symlink){ # file must exist, link must not
#    return {error=>"Cannot mark as done: remove '$symlink'?"};
#  }
  if(! -f $dst){ # file must exist, link must not
    return {error=>"Cannot add metadata: '$dst' does not exist"};
  }
  my $webPath=$self->SUPER::file_to_web($newFn);
  return {status=>201, location=>$webPath->{source}}; # success    
#  if(symlink($dst, $symlink)){
#    my $webPath=$self->SUPER::file_to_web($newFn);
#  }
#  return {error=>'Error creating metadata'};
}
sub update_metadata{
...
sub new_filename{
  ...
  return(
    $splitFn->{dir}. '/'. $fn. $id. $splitFn->{extn},
    $fn. $id. $splitFn->{extn}
  );
}
...
sub logger{
  my $logmsg=join ' ', @_;
  open (LOG, ">>/tmp/h5v.log") or die "$! check logfile\n";
  print LOG time. "\t". $logmsg. "\n";
  close LOG;
}
1;
